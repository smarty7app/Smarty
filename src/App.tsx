...
export default function App() {
  ...
  return (
    <div className={`flex h-screen ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-[#FAFAFA] text-slate-900'} font-sans selection:bg-orange-500/10 transition-all duration-300 ${isDarkMode ? 'dark' : ''}`} dir="rtl">
      {/* Sidebar Overlay */}
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div 
      ...
      <motion.aside 
        ...
        className={`fixed lg:relative top-0 right-0 h-full bg-slate-900 text-white z-[70] overflow-hidden flex flex-col border-l border-white/5 shadow-2xl transition-colors duration-300`}
      >
                      ...
                      <div key={session.id} className="relative group/session">
                        {editingSessionId === session.id ? (
                          ...
                          <div 
                            ...
                            className={`flex items-center gap-4 w-full p-4 rounded-2xl transition-all cursor-pointer group hover:bg-white/5 ${activeSessionId === session.id ? 'bg-white/10 ring-1 ring-white/10 shadow-inner' : ''}`}
                            role="button"
                            tabIndex={0}
                          >
                            <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 transition-colors ${activeSessionId === session.id ? 'bg-orange-500 text-white' : 'bg-white/5 text-white/20 group-hover:bg-white/10 group-hover:text-white/40'}`}>
                                ...
                                {activeDropdownId === session.id && (
                                  ...
                                  >
                                    <button
                                      ...
                                      className="flex items-center gap-3 w-full px-4 py-3 text-xs font-black uppercase tracking-widest text-white/60 hover:bg-white/5 hover:text-white transition-colors"
                                    >
                                      <Pencil size={12} />
      ...
      <div className={`flex-1 flex flex-col h-full relative overflow-hidden ${isDarkMode ? 'bg-slate-950' : 'bg-[#FDFDFD]'}`}>
        ...
        <header className={`flex-shrink-0 flex items-center justify-between px-4 h-16 backdrop-blur-md border-b sticky top-0 z-50 transition-colors ${isDarkMode ? 'bg-slate-900/80 border-slate-800' : 'bg-white/80 border-slate-200'}`}>
          ...
          <div className="flex items-center gap-4">
             ...
             <div className={`hidden lg:flex items-center gap-3 px-5 py-3 rounded-2xl border text-[10px] font-black uppercase tracking-[0.2em] ${isDarkMode ? 'bg-white/5 border-white/10 text-white/40' : 'bg-slate-50 border-slate-200 text-slate-400 font-mono'}`}>
