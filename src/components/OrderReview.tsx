      ...
      <div className="flex flex-col gap-3">
          ...
          order.status !== 'delivered' && (
            <div className="grid grid-cols-2 gap-3">
              <button 
                ...
                className="py-3.5 bg-gradient-to-r from-purple-600 via-violet-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-all text-sm disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shadow-lg shadow-purple-500/20 hover:shadow-purple-500/30"
              >
              ...
              <button 
                type="button"
                onClick={() => setShowPreviewModal(true)} 
                className="py-3.5 bg-slate-100 hover:bg-slate-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 border border-slate-300 dark:border-zinc-800 text-slate-700 dark:text-zinc-100 rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-all text-sm cursor-pointer"
              >
        ...
        <div className="flex gap-3">
          <button 
            onClick={handleSave} 
            disabled={loading || hasInsufficientStock} 
            className="flex-1 py-4 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-black border border-transparent rounded-2xl font-bold flex items-center justify-center gap-2 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-md"
          >
            <Save className="w-5 h-5" /> {order.id ? t.update_order : t.save_order}
          </button>
          <button onClick={() => { setOrder(initialOrder); setScreen(order.id ? "dashboard" : "input"); }} className={`w-16 h-14 bg-slate-100 hover:bg-slate-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 border border-slate-300 dark:border-zinc-800 rounded-2xl flex items-center justify-center text-slate-600 dark:text-zinc-400 hover:text-slate-950 dark:hover:text-white cursor-pointer transition-all ${isRtl ? 'rotate-180' : ''}`}><ArrowRight /></button>
        </div>
      </div>
        </div>
      </div>
