...
export default function LandingPage({ 
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0B0F19] text-slate-950 dark:text-white font-sans selection:bg-purple-500/30 overflow-x-hidden transition-colors duration-300" dir={isRtl ? "rtl" : "ltr"}>
      ...
      {/* Dynamic Authentication Error / Warning Diagnostic Overlay */}
      <AnimatePresence>
        {authError && (
          <motion.div 
            initial={{ opacity: 0 }}
          ...
          >
            <motion.div 
              ...
              className="w-full max-w-lg bg-[#0B0F19] border border-zinc-800 rounded-3xl p-6 shadow-2xl text-white space-y-6 select-none relative overflow-hidden"
            >
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-red-500 via-amber-500 to-indigo-505" />
              
              <div className="flex items-start gap-4">
                <div className="p-3 bg-red-550/10 border border-red-500/20 text-red-400 rounded-2xl shrink-0">
                  <AlertCircle className="w-6 h-6 animate-pulse" />
                </div>
                <div className="space-y-1.5 flex-grow">
                  <h3 className="text-sm font-black tracking-tight" dir={isRtl ? "rtl" : "ltr"}>
                    {isRtl ? "إعادة محاولة تسجيل الدخول" : "Retry Sign-In"}
                  </h3>
                  <p className="text-[10px] text-zinc-400 leading-relaxed font-mono bg-black/55 p-3 rounded-xl border border-zinc-900 select-text overflow-x-auto max-h-24" dir={isRtl ? "rtl" : "ltr"}>
                    {isRtl 
                      ? "فشل في تسجيل الدخول. يرجى إعادة محاولة تسجيل الدخول مجدداً." 
              ...
              <div className="flex gap-2.5 justify-end pt-1">
                <button
                  ...
                  className="px-4 py-2.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 hover:border-zinc-700 text-zinc-400 hover:text-white rounded-xl text-[10px] font-black transition-all cursor-pointer"
                >
                  {isRtl ? "إغلاق" : "Close"}
                </button>
              </div>
