...
export default function Subscription({ user, userData, setScreen, t, isRtl, planLimits }: any) {
  ...
  return (
    ...
    >
      ...
      {/* Centralized SaaS Usage Metrics Board */}
      {userData && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" dir={isRtl ? "rtl" : "ltr"}>
          {/* Card 1: Orders and Storage */}
          <div className="bg-zinc-900/30 border border-zinc-850 rounded-[1.75rem] p-5 backdrop-blur-md relative overflow-hidden">
            <h4 className="text-xs uppercase font-bold text-zinc-500 tracking-wider flex items-center gap-1.5 mb-3">
              <ShoppingBag className="w-4 h-4 text-emerald-400" />
              {isRtl ? "الطلبيات والتخزين" : "Orders & Storage"}
            </h4>
            <div className="space-y-2">
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-400">{isRtl ? "الطلبيات المعالجة:" : "Orders Processed:"}</span>
                <span className="font-mono font-bold text-zinc-200">{userData.ordersProcessed ?? userData.orderCounter ?? 0}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-400">{isRtl ? "العناصر المخزنة:" : "Stored Items:"}</span>
                <span className="font-mono font-bold text-zinc-200">{userData.storageUsed ?? 0}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-zinc-900/30 border border-zinc-850 rounded-[1.75rem] p-5 backdrop-blur-md relative overflow-hidden">
            <h4 className="text-xs uppercase font-bold text-zinc-500 tracking-wider flex items-center gap-1.5 mb-3">
              ...
              {isRtl ? "استهلاك الذكاء الاصطناعي" : "AI Consumption"}
            </h4>
            <div className="space-y-2">
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-400">{isRtl ? "الرموز المستخدمة (Tokens):" : "Tokens Used:"}</span>
                <span className="font-mono font-bold text-zinc-200">{(userData.tokensUsed ?? 0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-400">{isRtl ? "تكلفة الميزات (تفكيك):" : "Calculated AI Cost:"}</span>
                <span className="font-mono font-bold text-violet-300">${(userData.aiCost ?? 0).toFixed(6)}</span>
              </div>
            </div>
          </div>

          {/* Card 3: Logistics and Billing */}
          <div className="bg-zinc-900/30 border border-zinc-850 rounded-[1.75rem] p-5 backdrop-blur-md relative overflow-hidden md:col-span-2 lg:col-span-1">
            <h4 className="text-xs uppercase font-bold text-zinc-500 tracking-wider flex items-center gap-1.5 mb-3">
              <Truck className="w-4 h-4 text-sky-400" />
              {isRtl ? "التوصيل والفوترة" : "Logistics & Billing"}
            </h4>
            <div className="space-y-2">
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-400">{isRtl ? "طلبات الشحن والتوصيل:" : "Shipping Requests:"}</span>
                <span className="font-mono font-bold text-zinc-200">{userData.shippingRequests ?? 0}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-400">{isRtl ? "آخر عملية شحن/فوترة:" : "Last Billing Date:"}</span>
                <span className="font-mono text-[10px] text-zinc-400">
                  {userData.lastBillingDate ? new Date(userData.lastBillingDate).toLocaleDateString(isRtl ? 'ar-EG' : 'en-US', {
                    year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit'
                  }) : (isRtl ? "لا يوجد تاريخ" : "N/A")}
                </span>
              </div>
            </div>
          </div>
