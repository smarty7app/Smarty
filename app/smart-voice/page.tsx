...
export default function SmartVoicePage() {
  ...
  useEffect(() => {
    ...
    const handleSWMessage = (event: MessageEvent) => {
      ...
      switch (type) {
        case 'DOWNLOAD_STARTED': setDownloadingModel(modelId); break;
        case 'DOWNLOAD_PROGRESS': setDownloadProgress(prev => ({ ...prev, [modelId]: progress })); break;
    ...
    const handleSWDownloadComplete = (event: CustomEvent) => {
      const { modelId, filename } = event.detail;
      setDownloadedModels(prev => new Set(prev).add(`model_${modelId}`));
      setDownloadingModel(null);
      setResponse(`✅ ${t('download_success')}: ${filename}`);
    };
    const handleSWDownloadProgress = (event: CustomEvent) => {
    ...
    window.addEventListener('sw-download-complete', handleSWDownloadComplete as EventListener);
    window.addEventListener('sw-download-progress', handleSWDownloadProgress as EventListener);
    return () => {
  ...
  useEffect(() => {
    ...
    const loadSavedModels = async () => {
      try {
        ...
        request.onsuccess = () => setDownloadedModels(new Set(request.result as string[]));
        const savedActiveModel = localStorage.getItem('active_ai_model');
        if (savedActiveModel) setActiveModel(savedActiveModel);
        const pendingDownload = localStorage.getItem('pending_download');
        if (pendingDownload) {
          const { modelId, filename } = JSON.parse(pendingDownload);
          setDownloadingModel(modelId);
          await sendNotificationViaSW('🔄 تحميل مستمر', `تم اكتشاف تحميل غير مكتمل لنموذج ${filename}. جاري الاستئناف...`, 'pending-download');
        }
  ...
  const requestNotificationPermission = async () => {
    ...
    if ('Notification' in window) {
      ...
      if (permission === 'granted') {
        ...
        await sendNotificationViaSW('مرحباً!', 'سنخبرك عند اكتمال تحميل النماذج', 'welcome');
      }
    }
  };
  ...
  const downloadModelWithSW = async (model: AIModel) => {
    ...
    localStorage.setItem('pending_download', JSON.stringify({ modelId: model.id, downloadUrl: model.downloadUrl, filename: model.filename, startedAt: Date.now() }));
    if (navigator.serviceWorker.controller) {
      navigator.serviceWorker.controller.postMessage({ type: 'START_DOWNLOAD', data: { modelId: model.id, downloadUrl: model.downloadUrl, filename: model.filename } });
      ...
      downloadModelFallback(model);
    }
  };

  const downloadModelFallback = async (model: AIModel) => {
    if (typeof window === 'undefined') return;
    setDownloadingModel(model.id);
    setDownloadProgress(prev => ({ ...prev, [model.id]: 0 }));
    ...
    try {
      const response = await fetch(model.downloadUrl, { signal: abortControllerRef.current.signal });
      ...
      if (reader) {
        while (true) {
          ...
          if (value) {
            ...
            if (total > 0) setDownloadProgress(prev => ({ ...prev, [model.id]: (received / total) * 100 }));
          }
        }
      }
    ...
    } finally { setDownloadingModel(null); abortControllerRef.current = null; }
  };
  ...
  const cancelDownload = () => {
    ...
    if (typeof window !== 'undefined' && navigator.serviceWorker.controller && downloadingModel) {
      navigator.serviceWorker.controller.postMessage({ type: 'CANCEL_DOWNLOAD', data: { modelId: downloadingModel } });
    }
  };
  ...
  const deleteModel = async (modelId: string) => {
    ...
    if (confirm(t('delete_confirm'))) {
      ...
      setDownloadedModels(prev => { const newSet = new Set(prev); newSet.delete(`model_${modelId}`); return newSet; });
